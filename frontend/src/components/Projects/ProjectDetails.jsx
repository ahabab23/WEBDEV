  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        const response = await fetch(`http://127.0.0.1:5000/api/posts/${numericId}`);
        ...
        <div className="flex flex-col gap-6 text-sm text-gray-700">
          <div>
            ...
            <p className="text-base font-medium">
              {project.published ? project.published.toLocaleDateString('en-US', {
                year: 'numeric',
                month: 'long',
                day: 'numeric'
              }) : 'Not specified'}
          ...
          <div>
            ...
            <p className="text-base font-medium">{project.category || 'Not specified'}</p>
          </div>
          <div>
            <h3 className="uppercase text-gray-500 font-semibold text-xs mb-1">Client:</h3>
            <p className="text-base font-medium">{project.client || 'Not specified'}</p>
          </div>
        </div>
      </div>
    </div>
